        ...
        */
        switch (response.fixed_header.control_type) {
            case MQTT_CONTROL_CONNACK:
                ...
                if(_Connect_wait == client->Connect_status)
                {
                    client->Connect_status = _connack;
                }
                break;
            case MQTT_CONTROL_PUBLISH:
                /* stage response, none if qos==0, PUBACK if qos==1, PUBREC if qos==2 */
            ...
            case MQTT_CONTROL_SUBACK:
                ...
                if(_Subscribe_wait == client->Subscribe_status)
                {
                    client->Subscribe_status = _suback;
                }
...
{ 
    ...
    *buf++ = connect_flags;
    uint16_t tmp = (uint16_t) MQTT_PAL_HTONS(keep_alive); 
    memcpy(buf,&tmp,2); 
    ...
    if (connect_flags & MQTT_CONNECT_WILL_FLAG) {
        ...
        tmp = (uint16_t) MQTT_PAL_HTONS(will_message_size);
        memcpy(buf,&tmp,2);
                                  ...
                                  uint8_t publish_flags)
{
    ...
    uint16_t tmp;
    if (inspected_qos > 0) {
        tmp = (uint16_t)MQTT_PAL_HTONS(packet_id);
	memcpy(buf,&tmp,2);
...
{    
    ...
    uint16_t tmp;
    memcpy(&tmp,buf,2);
    response->topic_name_size = (uint16_t) MQTT_PAL_NTOHS(tmp);
    ...
    if (response->qos_level > 0) {
        memcpy(&tmp,buf,2);
        response->packet_id = (uint16_t) MQTT_PAL_NTOHS(tmp);
                                 ...
                                 uint16_t packet_id) 
{
    ...
    uint16_t tmp = (uint16_t) MQTT_PAL_HTONS(packet_id);
    memcpy(buf,&tmp,2); 
    //*(uint16_t*) buf = (uint16_t) MQTT_PAL_HTONS(packet_id);
    buf += 2;
...
ssize_t mqtt_unpack_pubxxx_response(struct mqtt_response *mqtt_response, const uint8_t *buf) 
{
    ...
    uint16_t tmp;
    memcpy(&tmp,buf,2);
    packet_id = (uint16_t) MQTT_PAL_NTOHS(tmp);
...
ssize_t mqtt_unpack_suback_response (struct mqtt_response *mqtt_response, const uint8_t *buf) {
    ...
    uint16_t tmp;
    memcpy(&tmp,buf,2);
    mqtt_response->decoded.suback.packet_id = (uint16_t) MQTT_PAL_NTOHS(tmp);
...
ssize_t mqtt_unpack_unsuback_response(struct mqtt_response *mqtt_response, const uint8_t *buf) 
{
    ...
    uint16_t tmp;
    memcpy(&tmp,buf,2);
    mqtt_response->decoded.unsuback.packet_id = (uint16_t) MQTT_PAL_NTOHS(tmp);
...
ssize_t __mqtt_pack_str(uint8_t *buf, const char* str) {
    ...
    /* pack string length */
    uint16_t tmp = (uint16_t) MQTT_PAL_HTONS(length);
    memcpy(buf,&tmp,2); 
