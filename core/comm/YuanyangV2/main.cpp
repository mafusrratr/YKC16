/**
 * YuanyangV2 tcu_comm process entry
 * BY ZF
 */
...
{
    std::cout << "YuanyangV2 comm received signal: " << sig << std::endl;
    if (g_process) {
        g_process->stop();
    }
}
...
int main(int argc, char* argv[])
{
    ...
    std::cout << "=== YuanyangV2 Comm Process ===" << std::endl;
    ...
    if (!process.initialize(configPath)) {
        std::cerr << "Failed to initialize YuanyangV2 comm process" << std::endl;
        return 1;
    }
    if (process.start() != 0) {
        std::cerr << "Failed to start YuanyangV2 comm process" << std::endl;
        return 1;
    }
