...
struct CommConfig {
    ...
    uint8_t chargerAddress;           // NYC 1字节充电桩地址
    std::string assetCode;            // NYC资产编码
    ...
    std::string rsaPublicKey;         // NYC主站RSA公钥（PEM/DER HEX/模数HEX）
    ...
    int tcpHeartbeatSec;              // NYC心跳周期（协议固定20秒）
...
private:
    ...
    struct GunRuntimeData {
        ...
        uint16_t yxOtherFault;         // 遥信-otherFault
    ...
    // BY ZF: NYC上线状态机：请求RSA公钥 -> 设备认证 -> 在线心跳
    enum PlatformLoginState {
        LOGIN_IDLE = 0,          // 已连TCP，等待发起上线流程
        LOGIN_REQ_RSA_PUBLIC_KEY,// 周期发送0x01请求RSA公钥
        LOGIN_REQ_AUTH,          // 周期发送0x02设备认证
        LOGIN_REQ_FEE_MODEL,     // 02H确认后主动请求0x73计费模型，等待0x74
        LOGIN_ONLINE             // 已上线，维持0x07心跳与业务上送
    };
    ...
    std::vector<uint8_t> buildPlatformFrameEx(uint8_t cmd, const std::vector<uint8_t>& body, int seqOverride,
    ...
    bool sendPlatformFrameEx(uint8_t cmd, const std::vector<uint8_t>& body, int seqOverride,
                             int transferReasonOverride, int chargerAddrOverride,
                             int bodyCountOverride);

    // BY ZF: NYC业务信息体构造
    std::vector<uint8_t> buildRsaPublicKeyRequestBody() const; // 0x01 请求RSA公钥信息体
    std::vector<uint8_t> buildLoginRequestBody() const;      // 0x02 设备认证明文信息体
    std::vector<uint8_t> buildFeeModelRequestBody(uint8_t gunNoBcd) const; // NYC 0x73 请求计费模型，无信息体
    std::vector<uint8_t> buildHeartbeatBody();               // 0x03 心跳请求体
    std::vector<uint8_t> buildAllTelemetryBody(uint8_t& pileCount) const; // NYC 0x11 全遥测
    ...
    void reportTelesignalPeriodic();
    void reportTelemetryPeriodic();
    ...
    static std::array<uint8_t, 6> buildTelesignalValues(const GunRuntimeData& rd);
    ...
    bool tryUpdateRsaPubKeyFromResponse(const uint8_t* body, size_t bodyLen);
    ...
    bool publishPlugChargeAuthResult(uint8_t chargerAddr, uint8_t transferReason,
...
private:
    ...
    std::vector<std::array<uint8_t, 6> > m_lastTelesignalValuesByGun; // 每枪最近一次已发送遥信值
    std::vector<uint8_t> m_lastTelesignalValidByGun; // 每枪遥信位图是否已建立基线
    std::vector<uint8_t> m_lastFaultActiveByGun;     // 每枪最近一次18H是否处于故障态
    std::vector<uint16_t> m_lastFaultCodeByGun;      // 每枪最近一次18H上送的故障详情码
    ...
    std::array<uint8_t, 16> m_aesSessionKey;         // NYC一次性AES-128密钥
    bool m_aesSessionKeyReady;                       // 一次性AES密钥已完成认证并可用于业务加解密
    bool m_loginCryptoPrepared;                      // 当前登录轮次已完成密钥准备
    bool m_platformOnlineEventActive;                // 平台在线事件状态（完成登录流程后才为true）
    std::string m_rsaPublicKeyActive;                // 当前生效RSA公钥（可被平台下发更新）
