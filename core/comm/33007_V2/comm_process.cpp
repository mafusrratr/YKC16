/**
 * NBT33007 V2 tcu_comm process implementation
 * BY ZF
 */
...
namespace {
    ...
    static const uint8_t kCmdAllTelesignal = 0x05;
    static const uint8_t kCmdAllTelemetry = 0x0B;
    ...
    static const uint8_t kCmdFeeModelV1 = 0x50;
    ...
    static const int kLegacyYxPointsPerGun = 6;
    static const int kYxPointsPerGun = 16;
    static const int kLegacyYcPointsPerGun = 7;
    static const int kYcPointsPerGun = 16;
    static const int kYmPointsPerGun = 9;
    static const int kLegacyYmPoints = 2;
    ...
    {
        // BY ZF: NBT33007 address is low-16 station + high-16 device.
    ...
    bool connectWithTimeout(int fd, const struct sockaddr* addr, socklen_t addrLen, int timeoutSec)
    {
        // BY ZF: Keep platform TCP connect bounded so daemon watchdog is not starved.
        ...
        FD_SET(fd, &wfds);
        struct timeval tv;
        tv.tv_sec = timeoutSec > 0 ? timeoutSec : 3;
        tv.tv_usec = 0;
    ...
    const char* cmdName(uint8_t cmd)
    {
        ...
        case kCmdFeeModelV1: return "fee_model_v1";
...
bool CommProcess::doInitialize()
{
    ...
    m_logSender.info("init_completed", std::string("nbt33007_v2 gun_count=") + std::to_string(m_config.gunCount));
    return true;
}
...
bool CommProcess::loadConfig()
{
    ...
    m_config.mqttClientId = cfg.getString(section, "mqtt_client_id", "tcu_comm_33007_v2");
    ...
    if (m_rsaPublicKeyActive.empty()) {
        m_logSender.info("rsa_public_key_empty", "ignored_by_standard_33007");
    }
    return true;
}
...
bool CommProcess::initMqtt()
{
    if (!m_mqtt.init(m_config.mqttClientId, true)) {
        std::cerr << "[33007_V2] mqtt init failed" << std::endl;
        return false;
    }
    ...
    if (!m_mqtt.connect(m_config.mqttHost, m_config.mqttPort, m_config.mqttKeepalive)) {
        std::cerr << "[33007_V2] mqtt connect failed" << std::endl;
        return false;
    }
...
{
    if (rc != 0) {
        std::cerr << "[33007_V2] mqtt connect rc=" << rc << std::endl;
        return;
    }
...
void CommProcess::publishInitialSetConfig()
{
    for (uint8_t gun = 0; gun < m_config.gunCount; ++gun) {
        ...
        cJSON_AddStringToObject(data, "protocol", "33007_V2");
        cJSON_AddItemToObject(root, "data", data);
        char* text = cJSON_PrintUnformatted(root);
        if (text) {
...
bool CommProcess::connectPlatformTcp()
{
    ...
    if (::getaddrinfo(m_config.masterHost.c_str(), portText.str().c_str(), &hints, &res) != 0 || !res) {
        if (m_config.debugTcp) {
            std::cout << "[33007_V2][TCP] resolve_failed host=" << m_config.masterHost
                      << " port=" << m_config.masterPort << std::endl;
        }
    ...
    for (struct addrinfo* rp = res; rp; rp = rp->ai_next) {
        ...
        if (connectWithTimeout(fd, rp->ai_addr, rp->ai_addrlen, 3)) {
            break;
        }
        ::close(fd);
        fd = -1;
    ...
    if (fd < 0) {
        if (m_config.debugTcp) {
            std::cout << "[33007_V2][TCP] connect_failed host=" << m_config.masterHost
                      << " port=" << m_config.masterPort << std::endl;
        }
    ...
    ioTimeout.tv_sec = 3;
    ioTimeout.tv_usec = 0;
    setsockopt(m_tcpFd, SOL_SOCKET, SO_SNDTIMEO, &ioTimeout, sizeof(ioTimeout));
    setsockopt(m_tcpFd, SOL_SOCKET, SO_RCVTIMEO, &ioTimeout, sizeof(ioTimeout));
    m_platformConnected.store(true);
    m_tcpRxCache.clear();
    // BY ZF: Standard NBT33007 has no RSA/AES login; TCP connected means ASDU can be exchanged.
    ...
    if (!m_platformOnlineEventActive && !m_config.offlineRunMode) {
        m_platformOnlineEventActive = true;
        publishPlatformLinkEvent(true, "tcp_connected");
    }
    if (m_config.debugTcp) {
        std::cout << "[33007_V2][TCP] connected host=" << m_config.masterHost
                  << " port=" << m_config.masterPort << std::endl;
    }
...
void CommProcess::driveLoginStateMachine(const std::chrono::steady_clock::time_point& now)
{
    ...
    switch (m_loginState) {
    case LOGIN_IDLE:
        // BY ZF: Keep compatibility with the old state enum, but enter standard online mode directly.
        m_aesSessionKeyReady = true;
        m_loginCryptoPrepared = true;
        m_loginState = LOGIN_ONLINE;
        break;
...
std::vector<uint8_t> CommProcess::buildHeartbeatFrame(uint32_t control)
{
    ...
    if (m_config.debugTcp) {
        std::cout << "[33007_V2][TX_FRAME] kind=heartbeat"
                  << " lenField=" << len
                                                 ...
                                                 uint32_t sourceAddr)
{
    ...
    appendU16LE(frame, kCommonAddrDefault); // BY ZF: NBT33007 public address is fixed to 0.
...
bool CommProcess::sendFrame(const std::vector<uint8_t>& frame)
{
    if (m_tcpFd < 0 || frame.empty()) {
        if (m_config.debugTcp) {
            std::cout << "[33007_V2][TX_SKIP] reason="
                      << (m_tcpFd < 0 ? "tcp_not_connected" : "empty_frame")
                      << " len=" << frame.size() << std::endl;
        }
        return false;
    }
    if (m_config.debugTcp) {
        std::cout << "[33007_V2][TX] len=" << frame.size()
                  << " hex=" << toHex(frame.data(), frame.size()) << std::endl;
    }
    size_t sent = 0U;
    while (sent < frame.size()) {
        const ssize_t n = ::send(m_tcpFd, frame.data() + sent, frame.size() - sent, MSG_DONTWAIT);
        if (n > 0) {
            sent += static_cast<size_t>(n);
            continue;
        }
        if (n < 0 && (errno == EAGAIN || errno == EWOULDBLOCK || errno == EINTR)) {
            m_logSender.warn("platform_tx", "send_would_block");
        } else {
            m_logSender.warn("platform_tx", std::string("send_error_") + std::strerror(errno));
        }
        closePlatformTcp();
        return false;
    }
...
{
    const std::vector<uint8_t> tail;
    const std::vector<uint8_t> frame = buildAsduFrame(kCmdFeeModelV1, 0x01, kCotActive, tail,
                                                      packFrameAddr(m_config.stationAddr, deviceAddrFromGun(0)));
    const bool ok = sendFrame(frame);
    if (ok) {
...
{
    std::vector<uint8_t> tail = pending.tailPlain; // BY ZF: Standard 33007 control ack echoes original body.
...
bool CommProcess::sendFeeModelAck(const ParsedFrame& pf, bool ok)
{
    ...
    const std::vector<uint8_t> frame = buildAsduFrame(kCmdFeeModelV1, pf.vsq, ok ? kCotConfirm : kCotReject,
                                                      pf.tail,
...
void CommProcess::processPlatformPacket(const uint8_t* frame, size_t frameLen)
{
    ...
    if (m_config.debugTcp) {
        std::cout << "[33007_V2][RX_FRAME] cmd=0x" << std::hex << static_cast<int>(pf.cmd) << std::dec
                  << "(" << cmdName(pf.cmd) << ")"
    ...
    if (pf.cmd == kCmdFeeModelV1 && (pf.cot == kCotConfirm || pf.cot == kCotBurst || pf.cot == kCotActive)) {
        FeeModel feeModel;
        const bool ok = parseFeeModel050(pf, feeModel);
        if (ok) {
            for (size_t i = 0; i < m_feeModelByGun.size(); ++i) {
                m_feeModelByGun[i] = feeModel;
                if (i < m_gunRuntimeData.size()) {
                    m_gunRuntimeData[i].feeModelId = feeModel.feeModelId;
                }
            }
...
{
    (void)cmd;
    // BY ZF: The platform execution rules for NBT33007-2013 use plain ASDU bodies.
    out = plain;
    return true;
}
...
bool CommProcess::decryptTail(uint8_t cmd, const uint8_t* body, size_t bodyLen, std::vector<uint8_t>& out)
{
    ...
    // BY ZF: Standard NBT33007 has no business-body encryption.
...
bool CommProcess::parseRemoteControl041(const ParsedFrame& pf, uint8_t& gun, uint8_t& action, cJSON** outData)
{
    ...
    const size_t kStdControlLen = 3U + 1U + 1U + 1U + 4U + 4U + 4U + 4U + 16U;
    const size_t kYuanyangControlExtLen = 4U + 18U;
    if (pf.tail.size() < kStdControlLen) {
        return false;
    }
    ...
    std::string order;
    if (pf.tail.size() > pos) {
        // BY ZF: 33007_V2平台扩展区比远洋旧结构多8字节，订单号从旧4字节金额偏移后再右移8字节开始。
        const uint8_t* ext = &pf.tail[pos];
        const size_t extLen = pf.tail.size() - pos;
        if (extLen >= 4U) {
            maxAmount = readFloatLEValue(ext);
        }
        const size_t kOrderOffset = 12U;
        if (extLen >= kOrderOffset + 18U) {
            order.assign(reinterpret_cast<const char*>(ext + kOrderOffset), 18U);
        } else if (extLen >= kYuanyangControlExtLen) {
            // BY ZF: 极短旧帧才兼容远洋旧扩展：4字节金额 + 18字节订单号。
            order.assign(reinterpret_cast<const char*>(ext + 4U), 18U);
        }
    }
    action = static_cast<uint8_t>(actionRaw & 0x03U);
    const uint8_t vehicleType = static_cast<uint8_t>((actionRaw >> 2) & 0x3FU);
    const bool isDischargeStart = (action == 0x02U);
    const bool isWholePileCharge = (action == 0x03U);
    ...
    cJSON* data = cJSON_CreateObject();
    cJSON_AddNumberToObject(data, "startTime", static_cast<double>(nowMs()));
    cJSON_AddNumberToObject(data, "pointNo", pointNo);
    cJSON_AddNumberToObject(data, "remoteAction", action);
    cJSON_AddNumberToObject(data, "vehicleType", vehicleType);
    cJSON_AddNumberToObject(data, "controlInfo", controlInfo);
    cJSON_AddNumberToObject(data, "stopCondition", controlInfo & 0x1F);
    cJSON_AddNumberToObject(data, "chargeMode", isDischargeStart ? 2 : 1);
    cJSON_AddNumberToObject(data, "v2g", isDischargeStart ? 1 : 0);
    ...
    cJSON_AddNumberToObject(data, "mergeChargeFlag", isWholePileCharge ? 1 : 0);
    cJSON_AddNumberToObject(data, "feeModelNo", 1);
    if (gun < m_feeModelByGun.size() && isFeeModelReady(m_feeModelByGun[gun])) {
        appendFeeModelToStartJson(data, m_feeModelByGun[gun]);
    } else {
        m_logSender.warn("start_charge_fee_model_missing", "gun=" + std::to_string(static_cast<int>(gun)));
    }
    if (gun < m_gunRuntimeData.size()) {
        ...
        if (gun < m_feeModelByGun.size() && isFeeModelReady(m_feeModelByGun[gun])) {
            ...
            m_gunRuntimeData[gun].feeModelId = feeModel.feeModelId;
            m_gunRuntimeData[gun].feeTimeNum = static_cast<int>(feeModel.timeNum);
            m_gunRuntimeData[gun].feeSegments.clear();
            m_gunRuntimeData[gun].feeSegments.reserve(feeModel.timeSeg.size());
            for (size_t i = 0; i < feeModel.timeSeg.size(); ++i) {
                FeeSegmentData seg;
                seg.startTs = feeModel.timeSeg[i];
                seg.endTs = (i + 1U < feeModel.timeSeg.size()) ? feeModel.timeSeg[i + 1U] : "2400";
...
bool CommProcess::isFeeModelReady(const FeeModel& feeModel) const
{
    // BY ZF: 下发给逻辑层的启动命令必须带完整计费模型数组。
...
void CommProcess::appendFeeModelToStartJson(cJSON* data, const FeeModel& feeModel) const
{
    if (!data || !isFeeModelReady(feeModel)) {
        return;
    }
    ...
    const size_t count = static_cast<size_t>(feeModel.timeNum);
    for (size_t i = 0; i < count; ++i) {
...
bool CommProcess::parseFeeModel050(const ParsedFrame& pf, FeeModel& feeModel)
{
    if (pf.tail.size() < 4U) {
        return false;
    }
    ...
    const size_t need = 3U + 1U + static_cast<size_t>(timeNum) * 5U + 5U * 4U;
    ...
    chargePrice[1] = readU32LE(&pf.tail[pos]); pos += 4U; // BY ZF: 1 峰，单位10^-5元。
    chargePrice[2] = readU32LE(&pf.tail[pos]); pos += 4U; // BY ZF: 2 尖，单位10^-5元。
    chargePrice[3] = readU32LE(&pf.tail[pos]); pos += 4U; // BY ZF: 3 平，单位10^-5元。
    chargePrice[4] = readU32LE(&pf.tail[pos]); pos += 4U; // BY ZF: 4 谷，单位10^-5元。
    const uint32_t servicePriceAll = readU32LE(&pf.tail[pos]); pos += 4U; // BY ZF: 统一充电服务费，单位10^-5元。
    servicePrice[1] = servicePriceAll;
    servicePrice[2] = servicePriceAll;
    servicePrice[3] = servicePriceAll;
    servicePrice[4] = servicePriceAll;

    std::ostringstream id;
    id << "33007V1_" << static_cast<unsigned long long>(nowMs());
    feeModel = FeeModel();
    ...
    for (size_t i = 0; i < segments.size(); ++i) {
        ...
        // BY ZF: V1计费模型按时段展开给logger保存：chargeFee/serviceFee长度均等于timeNum。
...
std::vector<uint8_t> CommProcess::buildAllLegacyTelesignalTail() const
{
    std::vector<uint8_t> out = buildInfoAddrTail(0);
    for (size_t i = 0; i < m_gunRuntimeData.size(); ++i) {
        const GunRuntimeData& rd = m_gunRuntimeData[i];
        const std::array<uint8_t, 16> vals = buildTelesignalValues(rd);
        for (int point = 0; point < kLegacyYxPointsPerGun; ++point) {
            out.push_back(vals[static_cast<size_t>(point)]);
        }
    }
    return out;
...
std::vector<uint8_t> CommProcess::buildAllLegacyTelemetryTail() const
{
    std::vector<uint8_t> out = buildInfoAddrTail(0);
    for (size_t i = 0; i < m_gunRuntimeData.size(); ++i) {
        const GunRuntimeData& rd = m_gunRuntimeData[i];
        const std::array<int32_t, 16> vals = buildTelemetryValues(rd);
        for (int point = 0; point < kLegacyYcPointsPerGun; ++point) {
            appendI32LE(out, vals[static_cast<size_t>(point)]);
        }
    }
    return out;
...
std::array<int32_t, 16> CommProcess::buildTelemetryValues(const GunRuntimeData& rd) const
{
    ...
    std::array<int32_t, 16> vals = {{
        ...
        0,                                   // BY ZF: 7-15 are reserved in the NBT33007 execution rules.
        0,
        0,
        0,
        0,
...
std::vector<uint8_t> CommProcess::buildLegacyPulseTail() const
{
    ...
    for (int i = 0; i < kLegacyYmPoints; ++i) {
        ...
        if (!m_gunRuntimeData.empty()) {
            const GunRuntimeData& rd = m_gunRuntimeData[0];
            val = (i == 0) ? scaleToU32(rd.totalEnergy, 100.0)
                           : scaleToU32(rd.totalAmount, 100.0);
        }
        appendU32LE(out, val);
        out.push_back(0x00);
    }
...
void CommProcess::reportRuntimePeriodic()
{
    ...
    for (uint8_t gun = 0; gun < m_config.gunCount; ++gun) {
        ...
        if (changed) {
            sendFrame(buildAsduFrame(kCmdExtChangedTelesignal, kYxPointsPerGun,
                                     kCotBurst, buildChangedTelesignalTail(gun),
                                     packFrameAddr(m_config.stationAddr, kBroadcastDeviceAddr)));
    ...
    if (periodic) {
        const int legacyYcCount = static_cast<int>(m_config.gunCount) * kLegacyYcPointsPerGun;
        const int ycCount = static_cast<int>(m_config.gunCount) * kYcPointsPerGun;
        const int legacyYxCount = static_cast<int>(m_config.gunCount) * kLegacyYxPointsPerGun;
        ...
        sendFrame(buildAsduFrame(kCmdAllTelemetry, static_cast<uint8_t>(0x80 | std::min(127, legacyYcCount)),
                                 kCotBurst, buildAllLegacyTelemetryTail(),
                                 packFrameAddr(m_config.stationAddr, kBroadcastDeviceAddr)));
        sendFrame(buildAsduFrame(kCmdExtAllTelemetry, static_cast<uint8_t>(0x80 | std::min(127, ycCount)),
                                 kCotBurst, buildAllTelemetryTail(),
                                 packFrameAddr(m_config.stationAddr, deviceAddrFromGun(0))));
        sendFrame(buildAsduFrame(kCmdAllTelesignal, static_cast<uint8_t>(0x80 | std::min(127, legacyYxCount)),
                                 kCotBurst, buildAllLegacyTelesignalTail(),
                                 packFrameAddr(m_config.stationAddr, kBroadcastDeviceAddr)));
...
void CommProcess::sendTotalCallResponse()
{
    ...
    const int pulseCount = static_cast<int>(m_config.gunCount) * kYmPointsPerGun;
    sendFrame(buildAsduFrame(kCmdAllTelemetry, static_cast<uint8_t>(0x80 | std::min(127, legacyYcCount)),
                             kCotTotalCall, buildAllLegacyTelemetryTail(),
                             packFrameAddr(m_config.stationAddr, kBroadcastDeviceAddr)));
    sendFrame(buildAsduFrame(kCmdExtAllTelemetry, static_cast<uint8_t>(0x80 | std::min(127, ycCount)),
                             kCotTotalCall, buildAllTelemetryTail(),
                             packFrameAddr(m_config.stationAddr, deviceAddrFromGun(0))));
    sendFrame(buildAsduFrame(kCmdAllTelesignal, static_cast<uint8_t>(0x80 | std::min(127, legacyYxCount)),
                             kCotTotalCall, buildAllLegacyTelesignalTail(),
                             packFrameAddr(m_config.stationAddr, kBroadcastDeviceAddr)));
    sendFrame(buildAsduFrame(kCmdExtAllTelesignal, static_cast<uint8_t>(0x80 | std::min(127, yxCount)),
