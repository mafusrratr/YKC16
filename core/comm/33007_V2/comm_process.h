/**
 * NBT33007 V2 tcu_comm process
 * BY ZF
 */

#ifndef NBT33007_V2_COMM_PROCESS_H
#define NBT33007_V2_COMM_PROCESS_H
...
// BY ZF: NBT33007 V2 runtime config, loaded from /usr/app/config/tcu_comm.ini.
struct CommConfig {
    uint8_t gunCount;
    std::string mqttHost;
    int mqttPort;
...
private:
    ...
    bool parseFeeModel050(const ParsedFrame& pf, FeeModel& feeModel);
    bool parseRecordConfirm042(const ParsedFrame& pf, uint8_t& gun, cJSON** outData);
    void appendFeeModelToStartJson(cJSON* data, const FeeModel& feeModel) const;
    bool isFeeModelReady(const FeeModel& feeModel) const;
...
#endif // NBT33007_V2_COMM_PROCESS_H
