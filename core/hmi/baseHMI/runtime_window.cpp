...
static QString actionText(bool v2gMode)
{
    return v2gMode ? QString::fromUtf8("放电") : QString::fromUtf8("充电");
}

static QString energyText(bool v2gMode)
{
    return v2gMode ? QString::fromUtf8("放电电量") : QString::fromUtf8("充电电量");
}

static QString amountText(bool v2gMode)
{
    return v2gMode ? QString::fromUtf8("放电金额") : QString::fromUtf8("充电金额");
}

static QString durationText(bool v2gMode)
{
    return v2gMode ? QString::fromUtf8("放电时长") : QString::fromUtf8("充电时长");
}

static QString infoText(bool v2gMode)
{
    return v2gMode ? QString::fromUtf8("放电信息") : QString::fromUtf8("充电信息");
}

static QString stateText(const std::string &state, bool v2gMode)
{
    if (state == "IDLE") {
    ...
    if (state == "STARTING") {
        return actionText(v2gMode) + QString::fromUtf8("启动中...");
    }
    if (state == "CHARGING") {
        return actionText(v2gMode) + QString::fromUtf8("进行中");
    }
    if (state == "STOPPING") {
        return QString::fromUtf8("停止") + actionText(v2gMode) + QString::fromUtf8("中...");
    }
    if (state == "STOPPED") {
        return actionText(v2gMode) + QString::fromUtf8("已完成");
    }
    if (state == "ERROR") {
...
{
    if (state == "STOPPED") {
        return QString::fromUtf8("%1已停止\n%2: %3kWh\n%4: %5元\n%6: %7")
            .arg(actionText(v2gMode))
            .arg(energyText(v2gMode))
            .arg(formatEnergy(totalEnergy))
            .arg(amountText(v2gMode))
            .arg(formatMoney(totalAmount))
            .arg(durationText(v2gMode))
            .arg(formatChargeTime(chargedTime));
    }
    if (state == "CHARGING" || state == "STOPPING") {
        return QString::fromUtf8("%1: %2kWh\n%3: %4元\n%5: %6\n%7: %8V %9A\nSOC: %10%")
            .arg(energyText(v2gMode))
            .arg(formatEnergy(totalEnergy))
            .arg(amountText(v2gMode))
            .arg(formatMoney(totalAmount))
...
static QString stopReasonText(const RuntimeWindow::GunUiData &gun)
{
    ...
    return actionText(gun.v2gMode) + QString::fromUtf8("完成");
}
...
void RuntimeWindow::handleLogicEvent(uint8_t gun, const std::string &payload)
{
    ...
    const int v2gFlag = cJSON_IsObject(data) ? getInt(data, "v2g", -1) : -1;

    if (v2gFlag != -1) {
        QMutexLocker locker(&m_dataMutex);
        m_guns[gun].v2gMode = (v2gFlag != 0);
    }
    ...
    if (eventName == "state_change" && cJSON_IsObject(data)) {
        ...
        if (to && cJSON_IsString(to) && to->valuestring) {
            ...
            if (m_guns[gun].state == "IDLE") {
                m_guns[gun].v2gMode = false; // BY ZF: 回到空闲态后清掉上一笔的充放电模式，避免影响下一次待机展示。
            }
...
void RuntimeWindow::handlePileData(uint8_t gun, const std::string &payload)
{
    ...
    if (type && cJSON_IsString(type) && type->valuestring &&
        ...
        const int v2gFlag = getInt(data, "v2g", -1);
        ...
        m_guns[gun].soc = getInt(data, "soc", getInt(data, "vehicleSoc", m_guns[gun].soc));
        if (v2gFlag != -1) {
            m_guns[gun].v2gMode = (v2gFlag != 0); // BY ZF: 兼容上游在 pile/data 中补发 v2g 标志的场景。
        }
    }
...
void RuntimeWindow::refreshIdlePage(const std::vector<GunUiData> &guns)
{
    ...
    if (cell0 && guns.size() > 0) {
        ...
        cell0->setTips(idleCellText(g.totalEnergy, g.totalAmount, g.chargedTime,
                                    g.voltage, g.current, g.soc, g.state, g.v2gMode));
    }
...
void RuntimeWindow::refreshAuthorizePage(const GunUiData &gun)
{
    ...
    const QString action = actionText(gun.v2gMode);
    ...
    if (tips2) {
        if (isError) {
            ...
            tips2->setText(QString::fromUtf8("扫码启动%1").arg(action));
...
void RuntimeWindow::refreshChargingPage(const GunUiData &gun)
{
    ...
    setLabelText(m_chargingPage, "lblEnergyTitle", energyText(gun.v2gMode));
    setLabelText(m_chargingPage, "lblMoney_2", amountText(gun.v2gMode));
    setLabelText(m_chargingPage, "lblChargeTime_4", action + QString::fromUtf8("电压"));
    setLabelText(m_chargingPage, "lblChargeTime_3", action + QString::fromUtf8("电流"));
    setLabelText(m_chargingPage, "lblChargeTime_2", action + QString::fromUtf8("时间"));
    setLabelText(m_chargingPage, "lblRemainTimeLabel", QString::fromUtf8("剩余%1时间").arg(action));
    ...
    setLabelText(m_chargingPage,
                 ...
                 gun.cardOfflineActive
                    ? QString::fromUtf8("再次刷卡停止%1，请等待停止完成后取卡").arg(action)
                    : QString::fromUtf8("请操作手机停止%1").arg(action));
}
...
void RuntimeWindow::refreshStoppingPage(const GunUiData &gun)
{
    setLabelText(m_stoppingPage,
                 "lblTitle",
                 QString::fromUtf8("%1枪%2中").arg(gun.gun == 0 ? "A" : "B").arg(actionText(gun.v2gMode)));
...
{
    setLabelText(m_checkoutPage, "lblHint", QString::fromUtf8("本次%1已结束").arg(actionText(gun.v2gMode)));
    setLabelText(m_checkoutPage, "label_3", energyText(gun.v2gMode) + QString::fromUtf8(":"));
    setLabelText(m_checkoutPage, "label_6", actionText(gun.v2gMode) + QString::fromUtf8("时间:"));
    setLabelText(m_checkoutPage,
                 "label_4",
                 gun.v2gMode ? QString::fromUtf8("放电金额:") : QString::fromUtf8("消费金额:"));
