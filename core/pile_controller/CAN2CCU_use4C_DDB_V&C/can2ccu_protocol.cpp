...
int CAN2CCUProtocol::encodeMeterData(uint16_t voltage, uint16_t current)
{
    // BY ZF: 电表实时数据通过 0x18/0x4D 单帧发送，业务数据为“2字节电压 + 2字节电流”。
    return encodeMeterDataFrame(voltage, current);
}
...
int CAN2CCUProtocol::encodeMeterDataFrame(uint16_t voltage, uint16_t current)
{
    // BY ZF: 按需求发送 184DE08A 单帧，Data[0]=gunNo+1，Data[1..2]=0.1V，Data[3..4]=0.1A，剩余填0。
    if (m_sendCallback == nullptr) {
        std::cerr << "[CAN2CCU] Send callback not set\n";
        return -1;
    }
    uint8_t businessData[4];
    businessData[0] = static_cast<uint8_t>(voltage & 0xFF);
    businessData[1] = static_cast<uint8_t>((voltage >> 8) & 0xFF);
    businessData[2] = static_cast<uint8_t>(current & 0xFF);
    businessData[3] = static_cast<uint8_t>((current >> 8) & 0xFF);
    return sendSingleFrame(0x18, PGN_METER_DATA, businessData, 4);
}
