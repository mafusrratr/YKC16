...
bool PileControllerProcess::initMqtt()
{
    ...
    m_mqtt.setConnectHandler([this](int rc) {
        ...
        for (uint8_t i = 0; i < m_config.gunCount; i++) {
            ...
            std::ostringstream meterTopic;
            meterTopic << m_config.mqttTopicPrefix << "/meter/" << (static_cast<int>(gunNo - 1) + m_config.biasNo) << "/data";
            m_mqtt.subscribe(meterTopic.str(), 1);
        }
...
{
    int gun = -1;
    const bool isPileCmd = (topic.find("/cmd") != std::string::npos);
    const bool isMeterData = (topic.find("/data") != std::string::npos && topic.find("/meter/") != std::string::npos);
    if (isPileCmd) {
        if (!parseGunFromTopic(topic, "/pile/", gun)) {
            return;
        }
    } else if (isMeterData) {
        if (!parseGunFromTopic(topic, "/meter/", gun)) {
            return;
        }
    } else {
        return;
    ...
    if (isMeterData) {
        ...
        if (cJSON_IsObject(data)) {
            ...
            v = cJSON_GetObjectItem(data, "current");
            if (cJSON_IsNumber(v)) {
        ...
        if (hasVoltage && hasCurrent) {
            ...
            const uint16_t rawVoltage = static_cast<uint16_t>(std::lround(voltage * 10.0));
            const uint16_t rawCurrent = static_cast<uint16_t>(std::lround(current * 10.0));
            can->sendMeterData(rawVoltage, rawCurrent);
        }
        cJSON_Delete(root);
        return;
    }
...
{
    return parseGunFromTopic(topic, "/pile/", outGun);
}

bool PileControllerProcess::parseGunFromTopic(const std::string& topic, const std::string& marker, int& outGun) const
{
    outGun = -1;
