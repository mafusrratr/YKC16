...
void CAN2CCUProtocol::copyFixedVin(char* outVin, size_t outLen) const
{
    // BY ZF: VIN 字段固定为 17 字节 ASCII，VirtualVin 版本不使用独立配置
    if (outVin == nullptr || outLen == 0) {
        return;
    }
    memset(outVin, 0, outLen);
    const size_t vinLen = strlen(CAN2CCU_FIXED_VIN);
    const size_t copyLen = (vinLen < outLen) ? vinLen : outLen;
    memcpy(outVin, CAN2CCU_FIXED_VIN, copyLen);
}
...
int CAN2CCUProtocol::setVehicleAuthData(const TCU2CCU_CmdVehicleAuthData* cmdData)
{
    ...
    copyFixedVin(m_cmdVehicleAuthData.vin, sizeof(m_cmdVehicleAuthData.vin));
...
#if CAN2CCU_SINGLE_GUN_LOAD_MERGE_TEST
    // BY ZF: 测试用单枪负载模拟合并充，合并充场景仍按单枪启动值 0x01 下发
    businessData[3] = 0x01;
#else
...
int CAN2CCUProtocol::decodeVehicleIdFrame(uint32_t canId, const uint8_t* data, size_t dataLen)
{
    ...
    if (result == 0) {
        ...
        copyFixedVin(m_statusVehicleIdData.vin, sizeof(m_statusVehicleIdData.vin));
...
int CAN2CCUProtocol::decodeStartCompleteFrame(uint32_t canId, const uint8_t* data, size_t dataLen)
{
    ...
    if (result == 0) {
        ...
        copyFixedVin(m_statusStartCompleteData.vin, sizeof(m_statusStartCompleteData.vin));
